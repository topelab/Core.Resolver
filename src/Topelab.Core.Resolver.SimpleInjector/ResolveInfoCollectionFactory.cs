...
namespace Topelab.Core.Resolver.Unity
{
    public static class ResolveInfoCollectionFactory
    {
        ...
        internal static void Register(this Container container, ResolveInfoCollection resolveInfoCollection, Dictionary<string, ConstructorInfo> constructorsByKey)
        {
                    ...
                    case ResolveModeEnum.Factory:
                        switch (resolveInfo.ResolveLifeCycle)
                        {
                            case ResolveLifeCycleEnum.Transient:
                                container.Register(resolveInfo.TypeFrom, c => resolveInfo.Factory.Invoke(c.Resolve<IResolver>()), Lifestyle.Transient);
                                break;
                    ...
                    default:
                        switch (resolveInfo.ResolveLifeCycle)
                        {
                            case ResolveLifeCycleEnum.Singleton:
                                ...
                                {
                                    container.RegisterSingleton(resolveInfo.TypeFrom, resolveInfo.TypeTo);
                                }
                                break;
                            case ResolveLifeCycleEnum.Scoped:
                                if (resolveInfo.ConstructorParamTypes.Length > 0)
